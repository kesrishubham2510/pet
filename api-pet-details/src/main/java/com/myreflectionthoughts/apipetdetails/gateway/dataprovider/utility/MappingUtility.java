...
public class MappingUtility {
    ...
    public Pet mapToPet(AddPetDTO addPetDTO) {
        LogUtility.loggerUtility.logEntry(logger, "Initiating AddPetDTO --> Pet mapping...");
        Pet pet = modelMapper.map(addPetDTO, Pet.class);
        pet.setClinicCardStatus(ClinicCardStatus.NOT_APPLIED);
        LogUtility.loggerUtility.logExit(logger, "Successfully mapped AddPetDTO --> Pet");
        return pet;
    }

    public Pet mapToPet(UpdatePetDTO updatePetDTO) {
        LogUtility.loggerUtility.logEntry(logger, "Initiating UpdatePetDTO --> Pet mapping...");
        Pet pet =  modelMapper.map(updatePetDTO, Pet.class);
        LogUtility.loggerUtility.logExit(logger, "Successfully mapped UpdatePetDTO --> Pet");
        return pet;
    }

    public PetDTO mapToPetDTO(Pet pet) {
        LogUtility.loggerUtility.logEntry(logger, "Initiating Pet --> PetDTO mapping...");
        PetDTO petDTO = modelMapper.map(pet, PetDTO.class);
        petDTO.setClinicCardStatusMessage(pet.getClinicCardStatus().getMessage());
        LogUtility.loggerUtility.logExit(logger, "Successfully mapped Pet --> PetDTO.");
        return petDTO;
    }

    public DeletePetDTO createDeletePetDTO(String petId) {
        LogUtility.loggerUtility.logEntry(logger, "Initiating DeletePetDTO generation for pet:- "+petId);
        ...
        LogUtility.loggerUtility.logExit(logger, "Successfully generated DeletePetDTO for pet:- "+petId);
        return deletePetDTO;
    }

    public PetDTO setClinicCardStatus(PetDTO petDTO) {
        LogUtility.loggerUtility.logEntry(logger, "Setting up ClinicCardStatusMessage for pet:- "+petDTO.getId());
        petDTO.setClinicCardStatusMessage(
                ClinicCardStatus.valueOf(petDTO.getClinicCardStatus()).getMessage()
        );
        LogUtility.loggerUtility.logExit(logger, "ClinicCardStatusMessage set-up done");
        return petDTO;
    }

    private void addMappingConfigurations() {

        LogUtility.loggerUtility.logEntry(logger, "Initiating mapping configurations...");
        ...
        LogUtility.loggerUtility.logEntry(logger, "Initiating String --> Category mapping config for AddPetDTO --> Pet ");
        ...
        LogUtility.loggerUtility.logEntry(logger, "Initiating String --> Gender mapping config for AddPetDTO --> Pet ");
        addPetDTO_To_Pet_Mapper.addMappings(
        ...
        LogUtility.loggerUtility.logEntry(logger, "Initiating Gender --> String mapping config for Pet --> PetDTO ");
        ...
        LogUtility.loggerUtility.logEntry(logger, "Initiating Category --> String mapping config for Pet --> PetDTO ");
        pet_To_PetTDTO_Mapper.addMappings(
        ...
        LogUtility.loggerUtility.logEntry(logger, "Initiating ClinicCardStatus --> String mapping config for Pet --> PetDTO");
        pet_To_PetTDTO_Mapper.addMappings(
        ...
        LogUtility.loggerUtility.logEntry(logger, "Initiating String --> ClinicCard mapping config for UpdatePetDTO --> Pet ");
        ...
        LogUtility.loggerUtility.logEntry(logger, "Initiating String --> Category mapping config for UpdatePetDTO --> Pet ");
        updatePetDTO_To_Pet_Mapper.addMappings(
                mapper -> mapper.using(conversionUtility.string_To_categoryConverter).map(
        ...
        LogUtility.loggerUtility.logEntry(logger, "Initiating String --> Gender mapping config for UpdatePetDTO --> Pet ");
        updatePetDTO_To_Pet_Mapper.addMappings(
                mapper -> mapper.using(conversionUtility.string_To_genderConverter).map(
        ...
        LogUtility.loggerUtility.logExit(logger, "Successfully added mapping configurations");
    }
}
